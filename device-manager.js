...
module.exports = function(config) {
  var nodePath = path.join(__dirname, '../dist', config.platformPath, 'bin');
...
function setupDevice(config, device, callback) {
  ...
  exec('echo $PWD && ' + path.join(nodePath, 'npm') + ' --prefix=. install ' + device.connector, {cwd: devicePathTmp}, function(error, stdout, stderr) {
    if (error) {
      console.error(error);
    }
...
function startDevice(config, device) {
  ...
  var child = new (forever.Monitor)('start', {
    ...
    command: path.join(nodePath, 'npm')
  });
