...
var Gateblu = function(config, deviceManager, dependencies) {
  ...
  skynetConnection.on('message', function(message) {
    ...
    if (message.topic === 'device-delete') {
      self.deleteDevice(message.deviceUuid, message.deviceToken, _.noop);
      return;
    }
  });
  ...
  self.cleanup = _.once(function() {
    ...
    try {
      deviceManager.stopDevices(function(){
        process.exit();
      });
    } catch (error) {
  ...
  self.stopDevice = function(uuid) {
    ...
    skynetConnection.whoami({}, function(gatebluData) {
      ...
      var devices = gatebluData.devices;
